...
type clickupTool struct {
	...
	execute     func(ctx context.Context, args json.RawMessage, client *Client) (json.RawMessage, error)
	client      *Client
}

func (t *clickupTool) Definition() tools.ToolDef {
...
func (t *clickupTool) Execute(ctx context.Context, args json.RawMessage, _ tools.ToolContext) (json.RawMessage, error) {
	// Dry-run support
	if os.Getenv("BELUGA_DRY_RUN") == "true" {
		return json.Marshal(map[string]interface{}{
			"status":  "dry_run",
			"tool":    t.name,
			"message": "ClickUp tool dry-run: no API call made",
		})
	}
...
// RegisterTools registers all ClickUp tools with the given registry.
func RegisterTools(registry *tools.Registry, client *Client) error {
	clickupTools := []clickupTool{
		{
			...
			client:      client,
			execute: func(ctx context.Context, args json.RawMessage, c *Client) (json.RawMessage, error) {
				var params struct {
	...
	for i := range clickupTools {
		if err := registry.Register(&clickupTools[i]); err != nil {
			return err
		}
	}
	return nil
